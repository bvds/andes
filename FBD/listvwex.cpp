	...
	if (pImageList)
	{
		...
		if (rcItem.left<rcItem.right-1)
		{
			ImageList_DrawEx(pImageList->m_hImageList, lvi.iImage,
					pDC->GetSafeHdc(), rcIcon.left, rcIcon.top, 16, 16,
...
//	rcItem.right -= m_cxStateImageOffset;
	...
	//here is where we may possibly have greek lettering.
	//we need to parse out formatting characters and DrawText in correct font
	//Use DrawText from Greek Drawing class
//	dc.DrawText(pszText,-1,rcLabel,DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_VCENTER);
	CGreekText::DrawText(pDC, rcLabel, pszText);
...
LPCTSTR CListViewEx::MakeShortString(CDC* pDC, LPCTSTR lpszLong, int nColumnLen, int nOffset)
{
	...
	//compensate for extra greek formatting chars
	CString strFmtChars;
	CString strTemp = lpszLong;
	int nStartPos = 0;
	while (strTemp.Find('$', nStartPos) != -1)
	{
		nStartPos = strTemp.Find('$', nStartPos);
		nStartPos++;
		strFmtChars = strFmtChars + "$";
	}
	int nSubLen = 0;
	int nFmtLen = 0;
	if (!strFmtChars.IsEmpty())
	{
		nFmtLen = lstrlen(strFmtChars);
		nSubLen = pDC->GetTextExtent(strFmtChars, nFmtLen).cx;
	}

	if(nStringLen == 0 ||
		(pDC->GetTextExtent(lpszLong, nStringLen).cx + nOffset - nSubLen) <= nColumnLen)
	{
		return(lpszLong);
	}
	...
	for(int i = nStringLen-1; i > 0; i--)
	{
		...
		szShort[i] = 0;
		if((pDC->GetTextExtent(szShort, i).cx + nOffset + nAddLen - nSubLen)
			<= nColumnLen)
		{
			break;
		}
...
void CListViewEx::OnItemchanging(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
	if ((pNMListView->uNewState & LVIS_SELECTED) &&
			(GetListCtrl().GetItemData(pNMListView->iItem) == 0) )
		*pResult = 1;
	else
		*pResult = 0;
}
