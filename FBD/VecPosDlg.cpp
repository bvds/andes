// VecPosDlg.cpp : implementation file
//
...
CVecPosDlg::CVecPosDlg(CDrawObj*pObj, CWnd* pParent /*=NULL*/)
	: CDrawObjDlg(CVecPosDlg::IDD, pObj, pParent)
{
	//{{AFX_DATA_INIT(CVecPosDlg)
	//}}AFX_DATA_INIT
}


void CVecPosDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CVecPosDlg)
...
BEGIN_CTL_TBL(CVecPosDlg)
	...
	"OK",			IDOK,
	"Cancel",		IDCANCEL,
END_CTL_TBL(CVecPosDlg)

BEGIN_MESSAGE_MAP(CVecPosDlg, CDrawObjDlg)
	//{{AFX_MSG_MAP(CVecPosDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CVecPosDlg message handlers
BOOL CVecPosDlg::OnInitDialog()
{
...
// !!! Also need to support using for non-drawn relpos variable !!!
// Will happen when defining sought quantity. 

void CVecPosDlg::InitObjectDlg()
{
...
void CVecPosDlg::InitVariableDlg()
{
...
void CVecPosDlg::UpdateTempVector()
{
...
void CVecPosDlg::UpdateTempVariable()
{
...
void CVecPosDlg::OnOK()
{
	// Make sure all visible controls filled in	
	if (IsEmpty(&m_cboBody)) {
		theApp.DoWarningMessage("Please select the body or point whose position is being defined", this);
		return;
	}
	if (IsEmpty(&m_cboFrom)) {
		theApp.DoWarningMessage("Please select the origin with respect to which the position is defined", this);
		return;
	}
...
void CVecPosDlg::OnSelchangeZdir() 
{
	int nZDir = m_cboZDir.GetCurSel();
...
void CVecPosDlg::OnChangeVectorNameText() 
{
