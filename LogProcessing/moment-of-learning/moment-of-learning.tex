...
physics.  Then we use a multi-model approach to determine the relative
...
One can see if a particular change to the instructional materials or 
help-giving strategy results in greater learning and from this,
one might infer when learning has occurred.  It is more difficult
to infer that a change in student {\em behavior} has resulted in 
greater learning, since this is uncontrolled.
...
student actions as they work.  Instead of relying on a distant
pre-test or post-test, the experimenter can easily examine student
(or tutor system) activity in the immediate vicinity of the event of 
interest.  
A key ingredient for improving an
...
the student attempts some number of 
{\em steps}  when solving a problem.  
Usually a given step is associated
with creating/modifying a single user interface object (an equation, vector, etc.)  but
...
a given KC if the student needs to apply that skill
...
For each KC and student, we select all steps that involve application
of that KC that were available to that student, and mark each step as ``correct'' if
...
a single student's performance on a single KC is a bit sequence,
...
gratia} 001001101, for each student-KC sequence.  This bit string is then
...
bit string for a particular student-KC sequence is 00011011 (8
