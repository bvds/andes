...
approach works when applied to student data.
\end{abstract}
...
An ITS is a computer-based learning environment where students construct problem solutions on a user
interface and the system evaluates the student work at the step-level.
...
Our stated goal is to determine student learning as they progress
through a course.  What observable quantities should be used to
determine student mastery?  One possible observable is ``correct/incorrect steps,''  the 
...
consider a model that uses a large number of additional observables (including ``time to
complete a step'') and use correct/incorrect steps (as defined above)
to calibrate the additional observables.
...
desirable to eventually include other observables in our determination
of student learning.
