...
the goal of determining empirically at which steps 
learning has probably occurred.
...
pairs that contained more than 10 turns, reducing the number of 
...
a particular model is the best model in a given set of models.
...
discriminate between the models.  It is only over examining
hundreds of student-KC pairs that one might expect to see
one model favored over the others.  However, this is not what
we found:  we see that $P_{\rm bkt}$ is strongly disfavored 
over the other two.
...
by the three criteria listed in Section~\ref{model-criteria}.
In Section~\ref{multi-model}, we show how AIC-based methods
can yield probabilities for learning at a given step.
