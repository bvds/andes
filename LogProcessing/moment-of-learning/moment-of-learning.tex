...
\begin{figure}
  ...
  \caption{Histogram of number of student-KC pairs having
    a given number of steps in student dataset.}
...
We then calculat the corrected AIC score, AIC$_\mathrm{c}$~\cite{aicbook} 
for each fit.  
...
Since an average student-KC pair has only about 16 steps, we 
expect that AIC$_\mathrm{c}$ should not strongly
discriminate between the models.  Instead, we expect that
any statistically significant diffence will only appear 
...
with model selection must be resolved.
...
where $\Delta_L=1-\hat{g}-\hat{s}$ and $\hat{g}$ and $\hat{s}$
are the Maximum Likelihood estimators for $g$ and $s$ given
by submodel  $P_{\mathrm{step},L}(j)$.  We define $\Delta_1=0$.
...
a ``figure of merit'' $Q$ for determining how suitable a 
dataset is for determining learning for an individual
student-KC pair:
%
\begin{equation}
...
we expect one $w_L$ to be around one with associated $\Delta_L\approx 1$ so
that $Q\approx 1$.
