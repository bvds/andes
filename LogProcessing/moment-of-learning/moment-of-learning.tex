...
Schwarz~\citeyear{schwarz_estimating_1978}.  Burnham \&
Anderson~\citeyear[Sections~6.3 \& 6.4]{burnham_model_2002} explain
that BIC is appropriate in cases where the ``true'' model that
...
complicated) and becomes more complicated as a dataset is increased.
Although authors often quote both AIC and BIC values, there
...
This data set corresponds to a model of the form
%
\begin{equation}
...
However, the only difference in our results is that BIC favors
$P_\mathrm{logistic}$ more strongly over $P_\mathrm{step}$ and
$P_\mathrm{BKT}$.  In order to maintain consistancy with
our experimental results, Fig.~\ref{scatter1}, we will continue
to use AIC.  The use of AIC versus BIC will not affect our conclusions.
