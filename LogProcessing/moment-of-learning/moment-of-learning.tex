...
\begin{figure}
  ...
  \caption{Scatter plot of  Akaike weights for the three models, 
   ...
   The point where
   $P_\mathrm{step}=P_\mathrm{logistic}=P_\mathrm{BKT}=1/3$ is
   marked with a cross.
   The point where all three models fit the
   data exactly is marked with a circle.  
   The dashed line on the left represents points
   where $P_\mathrm{step}=P_\mathrm{BKT}$.  Finally, the line on 
...
fit to each of the three models, $P_\mathrm{step}(j)$,
$P_\mathrm{logistic}(j)$, and $P_\mathrm{BKT}(j)$ by maximizing the
...
Finally, we calculated the Akaike weights, $w_\mathrm{logistic}$,
 ...
 The Akaike weight represents the relative probability that a
 particular model in a given set of models most closely matches the
 true model that has actually generated the data.  The weights are
 normalized so that
%
\begin{equation}
...
Instead, we see the step model weakly favored over the logistic 
model and strongly favored over BKT.
Indeed, we find no data points where $w_\mathrm{step}< w_\mathrm{BKT}$,
although there is a noticable accumulation of points when
$w_\mathrm{step}= w_\mathrm{BKT}$.


Since the student-KC pairs contain an average of about 16 steps,
it is surprising that we find a strong bias of 

  Since
a student-KC pair contains only an average of about 16 steps, we
expect that AI should not strongly discriminate between
the models.  Instead, we expect that any statistically significant
difference will only appear after including hundreds of student-KC
pairs in our sample.  However, this is not what we found; instead, we
see that $P_\mathrm{BKT}$ is strongly disfavored over
$P_\mathrm{step}(j)$ and $P_\mathrm{logistic}(j)$.  Perhaps this
...
the known correct model $P_\mathrm{random}(j)$.  Thus, we would expect
...
Fig.~\ref{scatter2}.  This indicates that there are large non-leading
corrections to the AIC$_\mathrm{c}$ that have not been taken into
account.  Burnham \& Johnson conclude that there is more work to be
done in this area~\cite[p.~380]{burnham_model_2002}: ``Operating
characteristics of AIC-based model selection for count-type data need
more more study for small sample sizes.''
