...
We use the Akaike Information Criterion to determine which of the models best
...
particular problem-solving step.
\end{abstract}
...
At best, one might infer when learning has occurred if a change to 
the instructional materials or help-giving strategy at some point results in better post-test performance.  
...
In a laboratory setting, addressing these issues is a part of every-day 
experimental design.

Moving from the laboratory to a more realistic setting, such as in a classroom
study, there is much less control over what
happens during an experiment and there is necessarily
an extended time between any pre-test and post-test.  Thus, in
cases where an experimental intervention is applied briefly, any
experimental effect can easily be swamped by ``noise'' 
(uncontrolled effects). 

% Talk about studies by Heckler


The use of an intelligent tutor systems (ITS) facilitates 
a style of experimentation where there is continuous recording of
%  Need a good citation for ITS with fine-grain logging in literature. 
% "First paper" is Aleven and Ken?
student actions as they work in the classroom (or at home)~\cite{Aleven_???}.  
Instead of relying on a distant
pre-test or post-test, the experimenter can examine student
(or tutor system) activity in the immediate vicinity of the event of 
interest.  The key ingredient for this approach is to have a method
for determining whether a particular change in instruction or 
student behavior is, in fact, effective.  There are several
...
of effectiveness:   Did the student or tutor action precede the
student mastering a new skill?  Answering this question requires
the use of some sort of embedded assessment of student learning.
To design such an assessment one needs to choose one or more ``observables'' 
(measurable quantities that might correlate with student mastery of a skill)
and a model that predicts mastery based on these observables.

In this work, the observable will be ``correct/incorrect steps:''  the 
student's ability to apply a skill at a given problem-solving step 
without any errors or hints.  In principle, there are other
...
observables need some additional theoretical interpretation ({\em exempli gratia},
What is the relation between time taken and mastery?).  Baker, Goldstein, and
Heffernan~\citeyear{baker_detecting_2010} develop a model of learning
based on a Hidden Markov model approach.  In their model, they
consider a large number additional variables (including ``time to
complete a step'') and use correct/incorrect steps (as defined above) to calibrate a model containing these additional observables.  Inclusion of such additional
observables is a natural extension of the approach we will present
here.

To start our investigation, we will compare three different models of learning and examine
...
Second, we use a multi-model approach to predict the probability that 
...
predictions of skill mastery.


\subsection{Correct/Incorrect steps}

There are many observables that may be used
to determine whether a skill has been mastered; Table~1 of
\cite{baker_detecting_2010} lists 14 different observables.  
However, we will focus on correctness of a step.  Thus, we need to
define precisely what we mean by a step being correct.

A student attempts some number of 
{\em steps}  when solving a problem.  
Usually, a step is associated
with creating/modifying a single user interface object (writing an equation, drawing a vector,
...
A step can be an {\em opportunity} to learn
...
the student completes that step correctly without any associated errors or 
...
a single student's performance on a single KC can be expressed as a  sequence,
