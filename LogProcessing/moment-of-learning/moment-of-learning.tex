...
get  $Q=-0.002\pm0.002$, consistent with zero.
For the ``ideal'' dataset $\mathcal{I}$,
we expect the maximum value of $w_L$ to be nearly one 
with associated $\Delta_L\approx 1$ so that $Q\approx 1$.
Numerically, we get $Q=0.5240\pm0.0003$, which is a little lower
than expected due to the large number of student-KC pairs with just one
or two steps.  For the student dataset
$\mathcal{A}$, we obtain $Q=0.047\pm0.008$, which
is small, but significant.  Since $Q$ is 6 standard deviations from
