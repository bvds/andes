...
%
\begin{equation}
           \bar{\chi} = \sqrt{\sum_j^N 
          \frac{\left(\log w_L-\log \bar{w}_L\right)^2}{N}}
%
For the student dataset, we get $\bar{\chi}=0.867$ while for
the random order dataset, we get $\bar{\chi}=0.675$. 
This suggests that the multimodel approach can distiguish
between any ``good'' and ``bad'' steps at a modest 
level.
