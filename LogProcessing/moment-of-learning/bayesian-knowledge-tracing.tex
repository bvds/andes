...
Since its introduction by Corebett and 
Anderson~\citeyear{corbett_knowledge_1994}, 
...
Researchers have applied BKT in distinct ways:  The {\em Hidden Markov Model}
...
study the behavior of its solutions.  This will lead to some
constraints on possible model parameters.
...
%
\begin{equation}
         P_j(C) = P(G)\left(1-P_j\right) + \left(1-P(S)\right) P_j \; . 
          \label{pnc}
\end{equation}
%
...
\caption{The hidden Markov model form of BKT. 
...
\caption{Relation between $P(G)$ and $P_0$ for the hidden
...
zero and one.  
In particular, we demand that $0\le P_0\le 1$ and
 $0\le P(G) \le 1$.
If we further demand that learning is positive (which may or may not be
empirically justified), then we have the constraint $A>0$ or $P(G)<1-P(S)$.
...
\caption{
  Recursion relations for the Knowledge Tracing Algorithm, 
...
$\mbox{incorrect}_n$.  The equations can be combined to give:
%
\begin{eqnarray}
       P_j &=&
...
the numerator of each equation.  Thus, any rescaling of $1-P_j$ 
cannot be compensated for by a rescaling
...
$P_0$ is needed as output, then we suggest fixing $A$ by
...
some authors, but would be much easier to implement.

Our other result is that the functional form of the model corresponds 
