...
a skill to update the probability that the student has learned 
that skill.
...
(\ref{pnc}) define a hidden Markov model, where $P(L_j)$ is the 
``hidden'' variable.
...
combinations of $P(G)$ and $P(L_0)$ give exactly the same $P(C_j)$, but
...
\caption{
 ...
 by a dot.  Since the ``step $j$ correct'' curve is above the
  dashed line, Eqn.~(\ref{correct}) causes $P(L_j)$ to converge to the fixed point at 1.  Likewise, since 
   the ``step $j$ incorrect'' curve is below the line, 
   Eqn.~(\ref{incorrect}) causes $P(L_j)$ to converge to the lower fixed point~(\ref{incorrectfixedpoint}).
 }
 \label{p-recursion}
\end{figure}
...
the recursion relation for updating $P(L_j)$ 
...
\begin{eqnarray}
       ...
       \frac{P(L_{j-1})\left(1-P(S)\right)}{P(L_{j-1})\left(1-P(S)\right)+
                                       \left(1-P(L_{j-1})\right)P(G)} \\
        P\left(L_{j-1}|\mbox{incorrect}_j\right) &=& 
              \frac{P(L_{j-1}) P(S)}
              {P(L_{j-1}) P(S)+ \left(1-P(L_{j-1})\right)\left(1-P(G)\right)}
                        \\
       P(L_j) &=& P\left(L_{j-1}|\mbox{action}_j\right)+
...
\begin{eqnarray}
       P(L_j) =&
       1-\frac{\left(1-P(T)\right)\left(1-P(L_{j-1})\right)P(G)}{P(G)+\left(1-P(S)-P(G)\right)
        P(L_{j-1})}  \;,&\; \mbox{step $j$ correct} \label{correct}\\
      P(L_j) =& 1-\frac{\left(1-P(T)\right)\left(1-P(L_{j-1})\right)\left(1-P(G)\right)}
                                 {1-P(G)-\left(1-P(S)-P(G)\right) P(L_{j-1})}
                        \; ,&\; \mbox{step $j$ incorrect.} \label{incorrect}
\end{eqnarray}
%
These equations are plotted in Fig.~\ref{p-recursion}.
...
(\ref{incorrect}), by setting $P(L_j)=P(L_{j-1})$.  For Eqn.~(\ref{correct}), we
...
the numerator of each equation.  Thus, any redefinition of $1-P(L_j)$ 
cannot be compensated for by a redefinition
of the parameters in both  (\ref{correct}) and (\ref{incorrect}).

In order for $P(L_j)$ to remain in the interval $\left[0,1\right]$ 
...
\end{eqnarray}
%
If we instead choose parameters consistant with negative learning,
$P(S)+P(G)>1$, we find that the behavior of $P(L_j)$ becomes inverted: 
$P(L_j)$ decreases for correct steps and increases for incorrect steps.
