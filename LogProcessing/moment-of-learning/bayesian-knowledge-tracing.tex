...
The Bayesian Knowledge Trading model~\cite{corbett_knowledge_1994} has four parameters:
%
\begin{itemize}
...
we find $P(L_j|O_j)$,  the probability that the student has learned a skill given
student performance on previous
steps~\cite{corbett_knowledge_1994,baker_more_2008},  
where $O_j=\left\{o_1,\ldots,o_j\right\}$ is the student performance
on the first $j$ opportunities and $o_i\in O_j$ can be either correct or
incorrect.  This probability obeys the recursion:
%
\begin{eqnarray}
       P(L_{j-1}|O_j) &=&
...
(\ref{incorrect}), by setting $P(L_j|O_j)=P(L_{j-1}|O_{j-1})$.  For Eqn.~(\ref{correct}), we
...
The likelihood for HMMs are often computed using 
the forwards algorithm.  The forwards algorithm gives $P(L_j \cap O_j)$ and
$P(\neg L_j \cap O_j)$ as output.  Using the definition of conditional
probability, $P(L_j \cap O_j)=P(L_j|O_j) P(O_j)$, where $P(O_j)$ is
the likelihood that the HMM produces $O_j$.  Thus, 
%
\begin{eqnarray}
P(O_j)&=&P(L_j\cap O_j) +P(\neg L_j \cap O_j)  \;\; \mbox{and} \\
