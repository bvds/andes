...
% Page heads
% Doesn't seem to be used in JEDM
\markboth{}{}
...
\caption{
  Recursion relations Eqn.~(\ref{correct}) and 
  Eqn.~(\ref{incorrect}).
  ...
  dashed line, Eqn.~(\ref{correct}) causes $P_j$ to 
   ...
   to the fixed point~(\ref{incorrectfixedpoint}).
 }
 \label{p-recursion}
\end{figure}
...
the Identifiability Problem:  all four model parameter
affect model behavior separately.  Thus, it is imperative that
all four model parameters are determined in a sensible manner.
