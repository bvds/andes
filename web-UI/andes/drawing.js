...
define([
    ...
    window.andes.drawing = {
            ...
            items[group.id] = group;
                ...
                connect.connect(item.master,"onClick",this,function(item){
                    ...
                    if(item.buttonType == "checkbox"){
                        if(item.selected) {
                            var pos=array.indexOf(item.group.checked,item.value);
                        ...
                        var myId=item.id;
                        array.forEach(item.buttons,function(button){
                            if(button.id == myId){
        ...
        handleServerActions: function(data){
            ...
            //console.dir(data);
            var mods = [];
            array.forEach(data, function(obj, i){
                ...
                if(obj.action =="new-object"){
                    ...
                    if(t=="vector" || t=="line" || t=="ellipse" || t=="rect"){
                        ...
                        var butt = array.map(o.items,function(item){
                        ...
                        // of radio buttons.
                        var buttOnly = array.map(butt,function(x){return x.master;});
                        array.forEach(butt,function(x){x.master.buttons=buttOnly;});
                    ...
                    if(obj.tool && obj.style){
                        ...
                        if(disable){tool.disable();} else {tool.enable();}
                    }
            ...
            array.forEach(mods, function(obj){
                ...
                if(items[obj.id]){
                    ...
                    if(obj.type=='vector' || obj.type=='line'){
                        ...
                        array.forEach(items[obj.id].items,function(pair){
                            if(array.indexOf(obj.checked,pair.master.value)!=-1){
                                pair.master.select();
                            }else{
