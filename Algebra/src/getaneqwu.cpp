...
binopexp* getAnEqn(const string bufst, bool tight) {
  int k;
  DBG(cout << "GETANEQ [tight = " << ((tight) ? "t" : "f" )
      << "] on " << bufst << endl;);
  ...
  while (! toklist->empty()) { 
    ...
    toklist->pop();
    DBG(cout << "GETANEQ popped " << token << endl;);
    ...
    if (isanum(token)) {
      ...
      // Do we want to declare this dimensionless
    ...
    if (token.compare("(")==0) {
      if (exprstack.empty())
        throw(string("getaneq found ( with empty stack"));
      ...
      if (exprstack.empty()) {
        ...
        save->pretty(8);
        throw(string("getaneq found ( with only one item on stack"));
      }
      if (exprstack.top()->etype == fake) {
        ...
        cerr << "getaneq found ( when top of stack not expr )" << endl;
        ...
        throw(string("getaneq found ( when top of stack not expr )"));
      }
    }
    if (token.compare(")")==0) {
  ...
  if (exprstack.size() != 1) {
    ...
    string message(string("getaneq had ") + buf 
                   + string(" rather than 1 item at end\n"));
    throw(message);
  } else {        
    if (exprstack.top()->etype != binop)
      throw(string("getaneq: trying to return equation not a binop"));
    binopexp * tempbin = (binopexp *)exprstack.top();
    exprstack.pop();
    DBG(cout << "GETANEQ returns " << tempbin->getInfix() << endl;);
    return(tempbin);
  }
}

bool getCanonEqn(const string bufst) {
  binopexp * thiseq = getAnEqn(bufst,false);
/* this stuff is for when we build in rel errors correctly - on hold
 * if (thiseq->lhs->etype == physvart) {  // see if this is a given
 *   eqnumsimp(thiseq->rhs);
 *   if (thiseq->rhs->etype == numval) {  // okay, we have a given, adjust its
 *     int k = ((physvarptr *)thiseq->lhs)->varindex; // abserr field
 *     (*canonvars)[k]->value = ((numvalexp *)thiseq->rhs)->value;
 *     (*canonvars)[k]->abserr = ((numvalexp *)thiseq->rhs)->abserr;
 *     DBG( cout << "set " << (*canonvars)[k]->clipsname << " to "
 *   << (*canonvars)[k]->value << " =- " <<
 *   << (*canonvars)[k]->abserr << endl; );
 *   }
 * }
 */
