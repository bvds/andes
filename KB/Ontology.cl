...
;;	"the angle between the displacement of e between T1 and T2, and the electric force on e between T0 and T1" 
...
(def-qexp angle-between (angle-between orderless . ?vecs)
  ...
  ;;	      (FORCE ELECTRON UNSPECIFIED ELECTRIC TIME (DURING 1 2)))
  ;;
  ;;Issue: The redundant time information should be allowed in the student input. In the
  ;;current modification, all reduadant time information was removed from
  ;;expansion. It was because the modification was done outside (conjoin .. )
  ;;process. The ideal version would be using "allowed" tag instead of removal of
  ;;redundant time info. It should be done by modifying (conjoin ..) process.
  ;;
  ;:new-english((the) "angle between" (conjoin (or "and" "&") . (eval (erase-redundant-time ?vecs))))) 
  :new-english((the) "angle between" (eval (erase-redundant-time '(conjoin (or "and" "&") . ?vecs))))) 

;; (erase-redundant-time '((velocity time (during 1 2)) (velocity time (during 1 2))))
;; 	= '((velocity time nil) (velocity time (during 1 2)))
(defun erase-redundant-time (vecs)
  (let* ((vec-last (first (last vecs)))
         (vec-last-timeinfo (first (last vec-last)))
         (vec-last-time (first (last (butlast vec-last))))
	 (ret nil))
    ;(format t "vec-last: ~A~%" vec-last)
    ;(format t "vec-last-timeinfo: ~A~%" vec-last-timeinfo)
    ;(format t "vec-last-time: ~A~%" vec-last-time)
    (if (or (eq vec-last-time 'TIME) (string= vec-last-time "TIME"))
	; erase the time info in all the other elements in the list
	(setq ret (append (erase-time-from-last (butlast vecs) vec-last-timeinfo) 
			  (list vec-last)))
...
;; (erase-time-from-last '((velocity time (during 1 2))) '(during 1 2))
;; = '((velocity time nil))
;; (erase-time-from-last '((velocity time (during 1 2)) (velocity time (during 1 2))) '(during 1 2))
;; = '((velocity time nil) (velocity time nil))
(defun erase-time-from-last (vecs last-timeinfo)
  (let* ((vec-last (first (last vecs)))
         (vec-last-timeinfo (first (last vec-last)))
         (butlast-vec-last (butlast vec-last))
         (butlast-vec-last-nil (append butlast-vec-last (list nil)))
	 (ret nil))
    ;(format t "vec-last in erase-time-from-last: ~A~%" vec-last)
    ;(format t "vec-last-timeinfo in erase-time-from-last: ~A~%" vec-last-timeinfo)
    ;(format t "last-timeinfo in erase-time-from-last: ~A~%" last-timeinfo)
    ;(format t "butlast-vec-last in erase-time-from-last: ~A~%" butlast-vec-last)
    ;(format t "butlast-vec-last-nil in erase-time-from-last: ~A~%" butlast-vec-last-nil)
    (if (equal vec-last-timeinfo last-timeinfo)
	(setq ret (append 
			  (erase-time-from-last (butlast vecs) last-timeinfo)
			  (list butlast-vec-last-nil)))
	(setq ret vecs))
    ret
  )
)
