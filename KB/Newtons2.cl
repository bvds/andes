...
(defoperator write-cons-ke-elastic (?bodies ?t1 ?t2)
  ...
  (
   ...
   (map ?b ?bodies (variable ?ke2-var (kinetic-energy ?b :time ?t2))
	?ke2-var ?ke2-terms)
   ;; The solve isn't quite up to solving this kind of problem.  Instead,
   ;; invoke any problem-specific routine that gives an explicit answer
   (bind ?prob (problem-name *cp*))
   (setof (implicit-eqn ?ans-eqn (answer ?prob)) nil ?dont-care)
  )
  :effects (
