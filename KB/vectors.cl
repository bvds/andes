...
;
; In our simple relvel* problems, the velocity triples to which it applies are identified by the following
; statement in the givens:
;   (relvel-triangle body1 body2 body3)
; body1 should be the moving object of interest.
;
; These is done to tell us exactly which system of relative velocities we can relate. Otherwise, whenever the 
; sought were a relative velocity Vab, we would have to try many possibilities: for each possible third object c we 
; could choose, we would have three candidate equations, one with Vab in each of the three possible roles. Most of
; these would turn out to be blind alleys. It simplifies things just to tell us which we should use. 

; Note: need var named ?body* for nsh to recognize this arg as a principle's important body (to prompt to draw first).
