...
(defun clips-var->quant (var)
  (let ((args (split-sym var)))
   (case (first args)
     (dist   `(at (distance ,(cbody (second args))) 
                 ,(ctime (cddr args))))
     (height `(at (height ,(cbody (second args))) ; third arg = ref point
                 ,(ctime (cdddr args)))) 
     (ke     `(at (kinetic-energy ,(cbody (second args))) 
                 ,(ctime (cddr args))))
     (mag     (vector-mag (clips-vector (rest args))))
     (mass   `(mass ,(cbody (second args))))
     (ntwd   `(at (net-work ,(cbody (second args))) 
                 ,(ctime (cddr args))))
     (num    `(at (num-forces ,(cbody (fourth args))) 
                  1)) ; no time in CLIPS var, assume default time OK
     (phi     (vector-dir (clips-vector (rest args))))
     (speed  `(at (speed ,(cbody (second args)))
                 ,(ctime (cddr args))))
     (the    `(duration (during ,(fifth args) ,(sixth args))))
     (theta   (vector-dir (clips-vector (rest args))))
     (twd    `(at (work ,(cbody (second args)) ,(cbody (third args)))
                 ,(ctime (cdddr args))))
...
(defun clips-vector (args)
  ...
  (if (eql (first args) T)     
         `(at (force ,(cbody (second args)) ,(cbody (third args)) TENSION) 
	             ,(ctime (cdddr args)))
  ; else:
   (case (first args)
    (A     `(at (accel ,(cbody (second args)))
               ,(ctime (cddr args))))
    (ALPHA `(at (ang-accel ,(cbody (second args)))
               ,(ctime (cddr args))))
    (CF	   `(at (force ,(cbody (second args)) ,(cbody (third args)) 
                 ; contact force now NORMAL in some prbs, APPLIED in others
                  ,(if (member (problem-name *cp*) '(EXDT3A EXDT14A EXDT15A)) 
		     'NORMAL 'APPLIED))
	       ,(ctime (cdddr args))))
    (DISP  `(at (displacement ,(cbody (second args)))
               ,(ctime (cddr args))))
    (F     `(at (force ,(cbody (second args)) ,(cbody (third args)) 
		   ; should be kinetic friction, but wrong in EXS8A 
                   ,(if (eql (problem-name *cp*) 'EXS8A) 'STATIC-FRICTION
		      'KINETIC-FRICTION))
	       ,(ctime (cdddr args))))
    (GF    `(at (force ,(cbody (second args)) ,(cbody (third args)) APPLIED) 
	       ,(ctime (cdddr args))))
    (L     `(at (ang-momentum ,(cbody (second args)))
               ,(ctime (cddr args))))
    (N     `(at (force ,(cbody (second args)) ,(cbody (third args)) NORMAL) 
	       ,(ctime (cdddr args))))
    (NF    `(at (net-force ,(cbody (second args)))
               ,(ctime (cddr args))))
    (NTOR  `(at (net-torque ,(cbody (second args)) ,(cbody (third args)))
               ,(ctime (cdddr args))))
    (OMEGA `(at (ang-velocity ,(cbody (second args)))
               ,(ctime (cddr args))))
    (SF    `(at (force ,(cbody (second args)) ,(cbody (third args)) STATIC-FRICTION) 
	       ,(ctime (cdddr args))))
    (THETA `(at (ang-displacement ,(cbody (second args)))
               ,(ctime (cddr args))))
    (V     `(at (velocity ,(cbody (second args)) ) 
               ,(ctime (cddr args))))
    (W     `(at (force ,(cbody (second args)) ,(cbody (third args)) WEIGHT) 
	       ,(ctime (cdddr args))))
