...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; macro for building strings
;;(defmacro gen-sen (format args) ;; this causes a segmentation fault in nlg ???
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
(defparameter **NLG-TYPES**
    '(def-np				;; definite noun phrase ==> The force
      indef-np				;; indefinite noun phrase ==> A force
      pp				;; prepositional phrase ==> During time t0 to t1
      adjective				;; adjective ==> Normal force .. Weight force
      adj
      algebra            ;; list
      conjoined-defnp    ;; list
      disjoined-defnp    ;; list
      conjoined-indefnp  ;; list
      disjoined-indefnp  ;; list
      moment
      lower-case
      goal
      psm-exp
...
;;
(defun nlg-gerund (x)
  "Convert x to a gerundform string."
  (format nil ":: ~A ::" x))
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
(defun old-nlg (x &optional (type 'def-np)) 
  "Convert x to a statement."
  (Tell :nlg "Translating ~A of type ~A" x type)
  (if (member type **NLG-TYPES**)
      (if (consp x)
	  (cond
	   ((equal type 'psm-exp) 
		...
		(format nil "[PSM: ~A]" x)))
	   ((equal type 'psm-type) 
	     (or (nlg-find (PSMGroup-form x)*Ontology-PSMGroups* #'PSMGroup-Form #'PSMGroup-English)
	         (nlg-find (PSMClass-form x) *Ontology-PSMClasses* #'PSMClass-Form #'PSMClass-English)
		 (format nil "[PSM type: ~A]" x)))
	   ((and (equal type 'goal) (goalprop-exp-p x))
	    (nlg-find x *Ontology-GoalProp-Types* #'GoalProp-Form #'GoalProp-English))
	   ((equal type 'conjoined-defnp)
	    (if (< (length x) 2)
		(format nil "the ~A" (nlg (first x)))
	      (nlg-print-list x '|and| 'def-np)))
	   ((equal type 'disjoined-defnp)
	    (if (< (length x) 2)
		(format nil "the ~A" (first x))
	      (nlg-print-list x '|or| 'def-np)))
	   ((equal type 'conjoined-indefnp)
	    (if (< (length x) 2)
		(format nil "a ~A" (first x))
	      (nlg-print-list x '|and| 'indef-np)))
	   ((equal type 'disjoined-indefnp)
	    (if (< (length x) 2)
		(format nil "a ~A" (first x))
	      (nlg-print-list x '|or| 'indef-np)))
	   ((equal type 'algebra)
	    (let* ((str (format nil "~A" (subst-student-vars (pre2in x)))))
	      (format nil "~A" (subseq str 1 (- (length str) 1)))))
	   (t 
	...
	(cond
	 ((null type)
	  (format nil "~(~A~)" x))
	 ((variable-p x) ;; hope varname is English common noun like ?body
	  (format nil "~@[~*at ~]some ~A" (eq type 'pp) (var-rootname x)))
	 ((equal type 'def-np)
	  (format nil "the ~(~A~)" x))
	 ((equal type 'algebra)
	  (format nil "~A" (canonical-to-student x)))
	 ((equal type 'indef-np)
	  (format nil "a ~(~A~)" x))
	 ((equal type 'pp)
	  (if (numberp x)
	    ...
	    (format nil "at ~(~A~)" x)))
	 ((or (equal type 'adj) (equal type 'adjective))
	  (handle-adjective x))
	 ((equal type 'moment)
	  (format nil "T~(~A~)" (if (numberp x) (1- x) x)))
	 ((equal type 'lower-case)
	  (format nil "~(~A~)" x))
	 ((equal type 'goal)
	  (format nil "~(~A~)" x))
	 (t
	  (format nil "illegal second argument in (nlg ~A ~A)!?" x type))))
    ;;(match x with rule and bind rule variables to
    (format nil "!$ ~(~A~) $!" x)))
;;

(defun handle-adjective (x)
  (declare (special adjectives))
...
; Most of our body names are common nouns like "car", "block", "man", which require an article, so
; that is default in def-np, But some problems use "block1", "charge2", etc. which are proper names 
; that shouldn't have an article. In absence of a lexicon, we here use a simple heuristic to 
; distinguish proper names by testing whether name ends in a number. 
; We also treat single-character nouns as proper names, e.g. A, B, C, D for points.
; This will give wrong answer for the very few common nouns we might use such as "F-14" that end 
; in number, but that's preferable to being wrong on all the block1's etc.
...
; Our circuit elements are named R1, C1, etc. For these we want to suppress the default
; lower-casing of names done by def-np.  Similarly for single-character names A, B, C. 
; For now, we just apply this to any "proper names" of 1 or 2 characters.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
(defparameter adjectives   ; includes some non-adj ids needing special translations
